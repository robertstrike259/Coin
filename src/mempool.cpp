#include "mempool.h"
bool Mempool::add(const Transaction& t,const std::map<OutPoint,Coin>& view,int spendHeight,std::string& why){
  if(t.isCoinbase()){why="coinbase";return false;}
  if(!checkTx(t,why)) return false;
  if(!checkInputs(t,view,spendHeight,why)) return false; // existence, dup, maturity, pkh auth, sig
...
void Mempool::recheck(const std::map<OutPoint,Coin>& view,int spendHeight){
  std::lock_guard<std::mutex> l(m);
  std::vector<std::string> drop;
  for(auto&kv:txs){ std::string why; if(!checkInputs(kv.second,view,spendHeight,why)) drop.push_back(kv.first); }
